    ...
    public partial class Success : System.Web.UI.Page
    {
            ...
            try
            {
                if (!String.IsNullOrEmpty(Request.Form["status"]) && Request.Form["status"] == "VALID")
                {
                    ...
                    if (orderValidate == true)
                    {
                        ...
                        // No need : Automatic call IPN validation
                       // DAPop.InsertCollectionEntrySSL(result, narration, mrno, loginId);
                    }

                    // save the payment info

                    SavePayment(result, loginId, GetUserIP());
                }
                else
                {
                    WriteLogFile.WriteLog("Not Found Transaction Status");
        ...
        private int SavePayment(SSLCommerzValidatorResponse objPay, string customerid, string Ip)
        {
            ...
            try
            {
                ...
                status = DAPop.SavePaymentLogFromSSL(objPay, customerid, Ip, "Success_Validation");
            }
            catch (Exception ex)
            {
